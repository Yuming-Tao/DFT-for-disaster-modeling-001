...
def main():
    ...
    # 保存结果
    output_path = os.path.join(os.path.expanduser("~"), "Desktop", "1.jpg")
    result_img.save(output_path)
    print(f"拼接完成，保存至: {output_path}")
    # 在 main 函数最后添加以下代码
    if 'result_img' in locals():  # 确保画布已创建
        try:
            # 手动指定绝对路径（避免相对路径问题）
            manual_path = "C:\\Users\\HP\\Desktop\\manual_save.jpg"
            result_img.save(manual_path)
            print(f"手动保存成功：{manual_path}")
        except Exception as e:
            print(f"手动保存失败：{e}")
